...
3. 展开右侧 Agent RL 面板，说明奖励、Token、成本、策略资产、SHADOW/CANARY/ACTIVE
   和自动回退；同时展示 AI 自动评分的 pending、每日额度和失败冷却。
4. 调用 `GET /api/agent-evaluation/benchmark/metadata`，展示固定基准版本、SHA-256
   指纹和单双 Agent 标签分布。
5. 展示一次四路报告：传统 RAG、强制单 Agent、强制多 Agent、自适应 Agentic RAG。
6. 展示 RLAIF/RLVR 四臂消融报告中的配对 95% CI、胜平负、符号检验和非劣性门禁，
   再展示实验清单中的四个不同模型资产指纹，说明同一模型无法重复贴标签冒充消融；
   两条烟雾评测只能验链路，不能冒充有效量化结论。
...
- 为什么 AI Judge 默认不自动跑？模型评审有真实费用，必须先经过每日额度、持久化暂停和
  连续失败冷却；能力默认可观测，付费执行必须显式开启。
...
- “均值更高”不等于可靠提升；只有逐样本配对统计通过后，报告才允许给出可发布结论。
- 四臂实验需要四个真实训练资产；仓库只编排和核验证据，不会在本地把同一模型跑四次来
  伪造消融。
- 三份候选训练数据在导出阶段就被不同 Profile 隔离，静态 Reward 与完整 RLVR 也由云端
  Runtime 的实验臂变量真实切换；产物、配置、数据和函数代码全部进入 SHA-256 来源清单。
