...
Release 验收默认检查：所有运行完成、RLVR 硬门禁全过、RLVR 平均分至少 0.70、
RLVR 违规为 0、路由偏差为 0、超时为 0。调用全部完成但门禁失败时，结果仍会持久化，
进程以非零状态退出，不能直接进入扩量阶段。

规模化候选采集使用 `--minimum-qualified-seeds` 切换到 `collection` 门禁。此模式不会因
首个坏样本提前终止，而是在计划完成后按 Seed 聚合多轮结果；只有所有轮次都通过硬门禁、
无违规、路由匹配、无超时且每轮达到 `--minimum-qualified-rlvr` 的问题才进入高置信集合。
Manifest 会保留合格 ID、淘汰 ID 和原因统计。

## 建议的样本规模

- 阶段 A：8 个种子 × 2 轮，验证接口、费用、策略版本和单双 Agent；
- 阶段 B：50 个种子 × 2 轮，检查奖励分布、失败率和主题覆盖；
- 阶段 C：先采 150 个种子 × 2 轮，形成 300 条候选轨迹并要求至少 82 个合格唯一问题；
- 若首批筛选后不足 82 个，再使用剩余离线种子补采，不自动执行第二批；
