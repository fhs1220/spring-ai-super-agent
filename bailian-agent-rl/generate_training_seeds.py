...
SCHEMA_VERSION = "agent-rl-trajectory-seed-v1"
...
def make_seed(
    ...
    concepts = list(concepts_for(unit.title))
...
def generate_seeds(
    ...
    rejected_near = 0
    candidate_index = 0
    maximum_candidates = len(units) * len(SCENARIOS) * len(REQUESTS) * 3
    ...
    if len(seeds) < target:
        raise ValueError(
            f"Only generated {len(seeds)} clean seeds from "
            f"{maximum_candidates} candidates; requested {target}"
        )
    manifest_payload = {
        "schema_version": SCHEMA_VERSION,
        "dataset_role": DATASET_ROLE,
        ...
        "samples_per_source_section": {
            unit.title: sum(
                1
                for seed in seeds
                if seed["rollout_extra"]["source_provenance"][0]["section"]
                == unit.title
            )
            for unit in units
        },
